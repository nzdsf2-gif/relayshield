...
- `POST /v1/metered/brand-monitor`: **NEW** brand protection: scan the 494K+ distinct indicator corpus (5.8M+ sightings) for brand name patterns, phishing domains, malware C2 infrastructure, and dark web mentions ($0.35)
...
- `GET /v1/intel/telegram`: IOC lookup against live threat intelligence database (494K+ distinct indicators from 5.8M+ sightings; domains, IPs, URLs, hashes)
...
MSSPs operating at scale can query RelayShield's live IOC database directly via `GET /v1/intel/telegram`. The feed aggregates **494K+ distinct indicators** (5.8M+ sightings) from **20 authoritative threat intelligence feeds** and **95 monitored channels**: updated continuously. RelayShield tracks **3,800+ malware families** including QakBot, LummaC2, Emotet, TrickBot, RedLine, Vidar, Raccoon, and 3,743+ others. IOCs are enriched with threat actor attribution, confidence scoring, and MITRE ATT&CK technique mapping.
...
**Price-to-performance:** Enterprise threat intelligence platforms (Recorded Future, ThreatConnect) start at $30K to $300K/year. RelayShield delivers 494K+ queryable distinct indicators, drawn from 5.8M+ sightings, at **$499/month**: the same enrichment data your clients' enterprise competitors pay $5K+/month to access.
...
Both bundles are live, public listings. **This is usually the fastest path to a signature**, because a Marketplace purchase draws down your existing AWS committed spend. Procurement is not approving a new vendor; they are approving a line item against budget they have already committed. For an MSP that has to get a client's finance team to say yes, that difference is often the whole deal.

| Bundle | Price | What it covers |
|---|---|---|
| **[Core Identity Exposure](https://aws.amazon.com/marketplace/pp/prodview-zgdxyqfd63hog)** | $150/mo + per call | Has this identity already been compromised? |
| **[Agentic Attack Surface](https://aws.amazon.com/marketplace/pp/prodview-6p6csngrcg3zq)** | $299/mo + per call | What is the exposure created by running AI agents? |

**Core Identity Exposure** is the one most MSPs want first. Six endpoints, priced per call on top of the monthly access fee:

| Endpoint | Per call | What it answers |
|---|---|---|
| `breach` | $0.10 | Does this address appear in breach corpora and dark web credential dumps? |
| `infostealer` | $0.50 | Were these credentials harvested by stealer malware? |
| `domain` | $0.30 | Is someone typosquatting this client's brand? |
| `oauth-watchlist` | $0.30 | Are OAuth or API tokens for this identity circulating? |
| `crypto-intel` | $0.30 | Is this wallet or domain in the criminal corpus? |
| `sim-swap` | $0.25 | Has this number been SIM swapped recently? |

The six are built to be correlated rather than run in isolation. An address in a breach corpus is routine and generates a ticket nobody works. **The same address with a fresh stealer-log hit and recent SIM swap activity is an active account takeover in progress**, and telling those two apart is the entire point of buying the bundle rather than six separate lookups.

**One caveat we would rather you hear from us:** `sim-swap` currently returns a `503`. Carrier registration is in progress and the endpoint is not billed while it is unavailable. It is listed because it is part of the bundle, not because it works today. We will tell you when it is live.

**No AWS procurement relationship?** Both bundles are also purchasable directly by card at the same per-call rates, at api.relayshield.net/developers. Same product, two doors.

**Reselling it:** if you want to sell either bundle to your own clients through AWS Marketplace, that is a Channel Partner Private Offer and we can set one up. Talk to us.
