...
> ⚠️ **Business Tier MVP Gap — Pre-Revenue Build Items (April 2026):** Business Basic and Business Shield are priced on the promise of team visibility and admin control. Four items must ship before these tiers are defensible at their price points: (1) **`REMOVE` command** — admin offboards an employee; without this no business with staff turnover will stay subscribed; (2) **`STATUS` command** — WhatsApp-native admin dashboard showing seat usage, per-employee onboarding state, and breach counts; (3) **Admin breach co-notification** — admin receives a second alert when any employee's credentials are detected in a breach; (4) **Employee name on `ADD`** — `ADD +16175551234 John Smith` stores name for use in STATUS and admin notifications. These four items together close the gap between what the tier descriptions promise and what the product currently delivers.
...
- ✅ Open a dedicated business bank account — Relay Financial approved April 2026
...
| 4 | **Business bank account** — Open dedicated account. LLC liability shield is weakened if business and personal funds are commingled. | ✅ Approved — Relay Financial, April 2026 |
