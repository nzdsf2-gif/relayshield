...
**Live:** https://blog.relayshield.net/sender-recognition-is-not-authentication
**Rendered copy:** `hashnode_export/sender-recognition-is-not-authentication.md`
(that file is what `build_blog.py` reads; this file is the working draft plus the channel plan).

**Two passages were cut at the founder's direction immediately before publishing:** the "precision
here is not pedantry" sentence after the correction, and the "citation laundry" paragraph in the
coverage section. The post still names JUMPSEC as the source throughout and still says "we now
carry the campaign infrastructure JUMPSEC published", so it makes no detection claim. The explicit
"we did not detect this campaign, JUMPSEC did" disclaimer now survives only in the social copy.

**Figures verified live 2026-08-11 before publishing:** `malware=apt38` returns **83** rows
(60 domain / 10 ip / 13 sha256), confidence split 44 high / 39 medium, `reference` present on every
row; `lazarus`, `nukesped`, `bluenoroff` and `unc1069` all still 0. IOC corpus **5,410,874**
(copy: `5.4M+`), active criminal channels **89**.

**Subtitle:** A North Korea linked campaign turns every victim's Telegram session into the next round of lures. The contact list is not the target. It is the supply chain.
