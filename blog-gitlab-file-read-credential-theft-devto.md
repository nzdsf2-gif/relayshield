...
If you want the check before the argument, it is two commands and it runs
...
Bare `rsscan` reads `git diff --cached -U0` and matches 49 credential patterns
locally. No account, no API key, and no network call on the scanning path. For
CI, `rsscan --rev-range origin/main...HEAD` scans a commit range instead, which
is strictly a backstop: by the time CI runs, the secret is already in history.

It also would not have saved you from this CVE, and the post below says so in
its own voice, because the half that actually hurts here is the server's own
config rather than anything a pre-commit hook has ever seen.
...
[api.relayshield.net/developers](https://api.relayshield.net/developers?source=gitlab-cve-devto).
