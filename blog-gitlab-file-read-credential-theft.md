...
**UNVERIFIED from the container, and it must stay labelled.** `cve.org`, `cisa.gov`,
`docs.gitlab.com`, `nvd.nist.gov` and `rapid7.com` are ALL egress-blocked from the session that
wrote this, checked rather than assumed (all returned HTTP 000). So every CVE detail here comes
from the pasted BankInfoSecurity article and nothing else.

**Nothing in the published copy rests on a number I could not check.** The argument is the
mechanism (unauthenticated read leads to credentials), which the article states in watchTowr's own
words. Version strings, the CVSS score and the CISA deadline are all ATTRIBUTED in prose to the
party that published them rather than asserted by us, which is the house rule for a post built on
somebody else's reporting and is also what makes them safe to print unverified.

**Before publishing, confirm three things in a browser:**
1. CVE-2026-85706 on cve.org, for the score and the affected range.
2. The CISA KEV entry, for the date added and the due date.
3. GitLab's patch release page, for the three version numbers.
If any differ, fix the post BEFORE it goes out. The gap between writing and publishing is where a
false claim gets in, and this one names specific versions to people making an emergency change.

**The GitLab token formats were verified**, from gitleaks' own rule source at
`raw.githubusercontent.com/gitleaks/gitleaks/master/cmd/generate/config/rules/gitlab.go`, which is
reachable. That is the BLOCKED SOURCE WAS REACHABLE ALL ALONG rule: a vendor's docs being blocked
does not make the fact unavailable, and a scanner's implementation is better evidence than prose.

## What shipped alongside this post

Written the same day, because a post claiming we detect GitLab credentials had to be true first:

- `NHI_PATTERNS` in `relayshield_api.py`: 8 GitLab rules, 41 entries to 49.
- `rsscan/rsscan/patterns.py`: regenerated via `tools/sync_patterns.py`, `--check` in sync.
- `_NHI_PATS` in `relayshield_intel_monitor.py`: the same 8, so collection recognises them.
- `test_gitlab_token_pattern.py`: 9 tests, every guard proven by reintroducing its defect
  (ordering swapped, rule dropped from one table, severities disagreeing, `_ctx_key` gating).
- `gitlab-cve` banner and 7 channel aliases in `relayshield_developer_signup.py`, registered
  BEFORE the post ships.

**The claim in the post is therefore checkable**: `pip install rsscan` and run it against a file
containing `glpat-` plus twenty characters.

## Attribution keys, registered before publication

    canonical    gitlab-cve-blog
    Medium       gitlab-cve-medium
    dev.to       gitlab-cve-devto
    LinkedIn     gitlab-cve-linkedin
    Telegram     gitlab-cve-telegram
    Farcaster    gitlab-cve-farcaster
    Mastodon     gitlab-cve-mastodon

Every link points at `api.relayshield.net/developers?source=<key>`, never the bare host: a
`?source=` on the root is read by nothing and logs nothing under that key.

## Channel order and what changes per channel

Canonical on blog.relayshield.net FIRST, then Medium (import with the canonical URL, never paste),
then dev.to, LinkedIn, Telegram, Farcaster, Mastodon. Not X, not Hashnode.

**dev.to tags, four maximum and they must already exist:** `security`, `devops`, `opensource`.
A fourth, `gitlab`, is very likely present but UNVERIFIED; a tag that does not exist returns a 422
naming it, so if that happens drop it and publish with three.

    python3 tools/publish_devto.py blog-gitlab-file-read-credential-theft-devto.md --dry-run
