...
|---|---|---|---|---|
| 1 | [tgboard.com](https://tgboard.com/en/add) | `tgboard` | Not submitted | Web form, SEEN directly (screenshot of the form itself) |
...
**Key** `tgboard` · **State** Not submitted
...
```text
https://t.me/relayshield_bot?start=SRC_tgboard
```

THE BOT KEY IS BARE `tgboard` AND THE MINI APP KEY IS `tg-miniapp-tgboard`, WHICH LOOKS LIKE A VIOLATION OF ONE-KEY-PER-DESTINATION AND IS THE OPPOSITE. Measured 2026-09-20 by running the funnel's own regexes against one line: `acquisition source=tg-miniapp-tgboard` matches the BOT stage AND the DIRECTORY stage, because the BOT stage's pattern is (miniapp|tg-miniapp[a-z-]*). That stage answers exactly one question -- does the Mini App feed the bot -- and a directory arrival counted inside it destroys the answer, which is the widening its own comment forbids. Every other key in this file is bare for that reason and this one was not. The cost of two keys here is adding two numbers together; the cost of one was corrupting a metric that already works. RANKED FIRST AND IT IS THE ONLY ROW ON THIS LIST WHOSE FORM ANYBODY HAS ACTUALLY SEEN. Every other row's url came from a web search and is egress-blocked here; this one comes from the founder's screenshot of tgboard.com/en/add, signed in, on 2026-09-20. That is a primary source and it outranks eleven secondary ones. IT TAKES ALL THREE OF OUR SURFACES: Type tabs for Channels, Bots and Mini-apps (also Groups and Stickers, which we do not have). Same shape as tg.app, which is the one catalogue that has actually gone live for us, so the tg.app playbook applies: search the catalogue for an existing RelayShield entry FIRST, then submit the Mini App, the bot and the channel as three listings. THE KEY IS SHARED WITH miniapp_routes.json's tgboard row ON PURPOSE. tgboard is ONE destination and the rule is one key per destination; the funnel already separates a ?startapp= check from a ?start=SRC_ bot arrival by stage, so a second key would split one delta into two numbers that have to be added back together. This is the only row here that shares a key with a Mini App route, and it is because it is the only destination that lists both. A 'Promotion' nav item means it also sells placement. That does NOT make it a broker -- the free submission form is on the same page -- but it is the @telegtapps shape, so read whether the free listing actually appears in search before paying for anything. THE FETCH BUTTON IS THE HAZARD ON ALL THREE SUBMISSIONS: a form that fetches a t.me URL to pull metadata may normalise it and drop the query string, which removes the attribution silently while the listing still looks right. Check the saved link after every Fetch. MEASURED 2026-09-20: THE LINK FIELD REFUSES A QUERY STRING. Pasting https://t.me/relayshield_bot/idcheck?startapp=tg-miniapp-tgboard returns 'Invalid or private t.me link' before any fetch runs. A hard refusal rather than a silent strip, which is the better of the two failures because it cannot be mistaken for success. Two candidate causes with different fixes, tested in the field itself: the query string, or the two-segment Mini App path. If only the bare bot username is accepted then this catalogue keys on the BOT USERNAME like tg.app, the Mini App and bot listings are one slot, and the slot goes to the Mini App. ATTRIBUTION RECOVERY: a bare Mini App link is not zero -- sourceFor() falls back to the generic tg-miniapp, so those arrivals land in the generic bucket and it is the tgboard DELTA that is lost, not the traffic. The reliable carrier is any Website field on the listing, set to https://api.relayshield.net/developers?source=tg-miniapp-tgboard -- the /developers path, never the bare host, which reads ?source= with nothing and is FD-8's shape. The bot link will probably be refused the same way and THAT one matters more: a bot arrival with no ?start=SRC_ payload logs nothing and leaves no unmatched: row to find later. DO NOT SUBMIT A BOT LISTING HERE YET. The Mini App was accepted under the BARE BOT USERNAME on 2026-09-20 (the /idcheck path was rejected), so the Mini-apps listing already occupies this bot's slot and a Bots-tab submission is a duplicate rather than an addition. Wait until the approved card is live and look at what it renders as. The bare `tgboard` key above stays registered for the day a bot listing IS possible; a key costs nothing and an unregistered one costs four months of unmatched arrivals.
