# Hashnode cross-post — smolagents v2 feedback release

Paste the block below into Hashnode's editor with **Cmd+Shift+V (plain text)**, not a normal paste — a normal paste leaves the Markdown syntax unrendered as literal characters.

**Post settings / Discovery page:**
- **Article slug**: `smolagents-agent-security-tools-v2` (matches the HF post's slug)
- **Canonical URL**: `https://huggingface.co/blog/relayshieldadmin/smolagents-agent-security-tools-v2` (cross-post pointing back to HF, same pattern as the original)
- **Tags** (cap ~5): `ai`, `cybersecurity`, `mcp`, `agents`, `opensource`
- **SEO title** (~60 char budget): `smolagents Agent Security Tools v2 (9 Tools, Typed Results)`
- **SEO description** (~150-160 char budget): `Five new smolagents.Tool checks plus a typed outcome/recommended_action result schema, shipped in direct response to community feedback on the original release.`
