...
def _parse_passwords_file(text: str, channel: str = "") -> list[dict]:
    ...
    _NHI_PATS = [
        ...
        # BOT-TOKEN-1, 2026-09-13. THIS IS THE COPY THAT DECIDES WHETHER THE
        # CORPUS EVER CONTAINS ANY. The source of truth is NHI_PATTERNS in
        # relayshield_api.py; rsscan's mirror is generated by
        # tools/sync_patterns.py; this one is hand-kept and is the one that has
        # drifted before. Two entries, and the second exists because the first
        # cannot see the most common leak shape -- a token in a URL, which
        # carries no assignment operator. Proven by running both against real
        # strings rather than by reading them.
        ("telegram_bot_token",
         _ctx(r"telegram|tg_bot|bot_?token|TELEGRAM_BOT_TOKEN|api\.telegram\.org",
