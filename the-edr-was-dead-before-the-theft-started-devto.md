...
published: false
description: "Rapuncel disables 145 security products with a Microsoft-signed driver before it steals anything. What survives when the detection layer is switched off first."
tags: security, devops, opensource
canonical_url: https://blog.relayshield.net/the-edr-was-dead-before-the-theft-started
---

Before the argument, something you can run. Rapuncel arrived through GitHub repositories impersonating real projects, so this screens a repository URL before you trust it, against Google Safe Browsing, a criminal indicator corpus and domain age:

```bash
curl -sS -X POST https://api.relayshield.net/v1/link-check \
  -H "Content-Type: application/json" \
  -d '{"url":"https://github.com/some-repo-you-are-about-to-trust"}'
```

No key, no card, no signup. It is capped per source IP rather than billed, and it will never tell you something is safe: the ceiling on a clean answer is "nothing known against it".
...
The API is at
[api.relayshield.net/developers](https://api.relayshield.net/developers?source=rapuncel-devto).
