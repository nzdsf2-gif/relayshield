#!/bin/sh
# relayshield_discord_bot.py: verify the live function name and READ THE FIRST
# DRIFT DIFF, without waiting for the nightly check.
#
...
FILE=relayshield_discord_bot.py
MAPPED=relayshield-discord-bot
...
if [ ! -f "$FILE" ]; then
  ...
  echo "  cd ~/\"Side SaaS Hustle\" && sh tools/discord_bot_drift.sh" >&2
  exit 1
fi
...
echo "== 2. What is the Discord bot's live function actually called?"
if aws lambda get-function-configuration --function-name "$MAPPED" \
  ...
  echo "   Searching every function for 'discord'..."
  CANDIDATES=$(aws lambda list-functions \
                 --query 'Functions[*].FunctionName' --output text \
               | tr '\t' '\n' | grep -i discord || true)
  if [ -z "$CANDIDATES" ]; then
    echo
    echo "STOP: no Lambda in $ACCOUNT has 'discord' in its name." >&2
    echo "Either the bot runs under an unrelated name, or it is not deployed at" >&2
    echo "all -- which would explain the missing footer line exactly as well as a" >&2
    echo "stale deploy does. Check the Function URL configured in the Discord" >&2
    echo "developer portal as INTERACTIONS ENDPOINT URL; the URL's host contains" >&2
...
if diff -q "$FILE" "$WORK/live_handler.py" >/dev/null; then
  ...
  echo
  echo "   Which means the email-check footer IS live. If Discord is not showing"
  echo "   it, the cause is downstream of the code: a cached command response,"
  echo "   or /scan being answered by a different function."
  exit 0
fi
  ...
  cat <<'VERDICT'
   ...
   RECOVER IT FIRST:
     ...
     Actions -> Recover Live Lambda Handler -> Run workflow
       ...
       handler:  relayshield_discord_bot.py
